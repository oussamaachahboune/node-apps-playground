# node-apps-playground

Building 4 Node.js apps from scratch.

![Node.js](https://img.shields.io/badge/Node.js-18+-green)
![Status](https://img.shields.io/badge/Status-In%20Progress-blue)

## Apps

| App            | Focus                                                               | Status         |
| -------------- | ------------------------------------------------------------------- | -------------- |
| Note Taking    | fundamentals, core Node.js concepts, basic features                 | 🔲 Not Started |
| Weather App    | web servers, APIs, production deployment, third party communication | 🔲 Not Started |
| Task Manager   | authentication, MongoDB, database and data persistence              | 🔲 Not Started |
| Real Time Chat | Socket.IO, WebSockets, real time communication                      | 🔲 Not Started |
